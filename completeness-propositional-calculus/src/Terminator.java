...
public class Terminator {
    ...
    public static ArrayList<Expression> terminate(ArrayList<Expression> derivation1, ArrayList<Expression> derivation2, int mask, int kth, int total) {
        String p;
        String np;
        char ch = (char) ('A' + kth);

        Expression alphaExpr = derivation1.get(derivation1.size() - 1); // or derivation 2, never mind
        String alpha = alphaExpr.s;

//        String alpha = null;
//        for (int i = derivation1.size() - 1; i >= 0; --i) {
//            alpha = derivation1.get(i); // or derivation 2, never mind
//            if (alpha.charAt(0) != '#')
//                break;
//        }
...
//            System.out.println("alpha is" + alpha + "!!!!");
        ...
        Expression pExpr = Parser.parse(p);
        Expression npExpr = Parser.parse(np);


        ArrayList<Expression> res = new ArrayList<Expression>();
        ArrayList<Expression> gamma = new ArrayList<Expression>();

        // добавим в гамму неизменные переменные(те, что все еще - предроложения)
        for (int i = kth+1; i < total; ++i)                 // все, что больше num или поглотятся не здесь или не существуют
        {
            if (Expression.getbit(mask, i))
                gamma.add(Parser.parse((char) ('A' + i) + ""));
            else
                gamma.add(Parser.parse("!(" + (char) ('A' + i) + ")"));
        }

        // res.add("#terminate " + (char) ('A' + num) + "{");

        res.addAll(deduction(derivation1, pExpr, gamma));
        res.addAll(deduction(derivation2, npExpr, gamma));

        res.addAll(lemma45(p));

        res.add(Parser.parse(("(" + "(" + p + "->" + alpha + ")" + "->" + "(!(" + p + ")" + "->" + alpha + ")" + "->" + "(" + p + "|!(" + p + ")" + ")" + "->" + alpha + ")")));
        res.add(Parser.parse(("(" + "(!(" + p + ")" + "->" + alpha + ")" + "->" + "(" + p + "|!(" + p + ")" + ")" + "->" + alpha + ")")));
        res.add(Parser.parse("(" + "(" + p + "|!(" + p + ")" + ")" + "->" + alpha + ")"));
        res.add(Parser.parse(alpha));
...
//                    }
        ...
        return null; // если исходный вывод корректен, никогда не вернется отсюда.
    }
    ...
    private static boolean Axiom2(Expression e) {
        ...
        if (e.getClass() == Impl.class &&
                ((BinaryOperation) e).lhs.getClass() == Impl.class &&   // левый импл
                ((BinaryOperation) e).rhs.getClass() == Impl.class &&   // правый импл
