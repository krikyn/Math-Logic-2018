...
public class z2 {
    ...
    public static ArrayList<String> rebuildProof(ArrayList<String> currentProofList) {

        HashMap<Expression, ArrayList> mpParts = new HashMap<>(53000, 1);
        HashMap<Expression, Integer> assumptions = new HashMap<>(53000, 1);
        ArrayList<Expression> storeExpr = new ArrayList<>(53000);

        HashMap<Expression, Integer> lines = new HashMap<>(53000, 1);
        ArrayList<String> result = new ArrayList<>();
        StringBuilder sb = new StringBuilder();

        String[] firstLine = currentProofList.get(0).replace(" ", "").split("\\|-");

        String[] exprs = firstLine[0].split(",");
        for (int i = 0; i < exprs.length - 1; i++) {
            Expression newExpr = Parser.parse(exprs[i]);
            assumptions.put(newExpr, i);
            sb.append(newExpr.s + (i < exprs.length - 2 ? "," : ""));
        }
        ...
        int curNum = 0;
        //String line;
        for (String line: currentProofList.subList(1, currentProofList.size())){
        //while ((line = in.readLine()) != null) {
            ...
            switch (caseNum) {
                ...
                case 2:
                    ...
                    result.add("(" + aP + "->(" + aP + "->" + aP + "))");
                    //out.newLine();
                    result.add("((" + aP + "->(" + aP + "->" + aP + ")" + ")" + "->" + "((" + aP + "->((" + aP + "->" + aP + ")->" + aP + "))" + "->" + "(" + aP + "->" + aP + ")))");
                    //out.newLine();
                    result.add("((" + aP + "->((" + aP + "->" + aP + ")->" + aP + "))" + "->" + "(" + aP + "->" + aP + "))");
                    //out.newLine();
                    result.add("(" + aP + "->((" + aP + "->" + aP + ")->" + aP + "))");
                    //out.newLine();
                    result.add("(" + aP + "->" + exprP + ")");
                    //out.newLine();
                    break;
                case 3:
                    ...
                    result.add("((" + aP + "->" + additionally + ")" + "->" + "((" + aP + "->(" + additionally + "->" + exprP + "))" + "->" + "(" + aP + "->" + exprP + ")))");
                    //out.newLine();
                    result.add("((" + aP + "->" + "(" + additionally + "->" + exprP + "))" + "->" + "(" + aP + "->" + exprP + "))");
                    //out.newLine();
                    result.add("(" + aP + "->" + exprP + ")");
                    //out.newLine();
                    break;
