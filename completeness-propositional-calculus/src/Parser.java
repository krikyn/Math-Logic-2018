public class Parser {
    public static Expression parse(String s) {
        ...
        if (s == null || s.length() == 0) {
            System.out.println("RTURNED NULL FROM PARSER!!!");
            return null;
        }
        ...
        for (int i = 0; i < s.length() - 2; ++i) {
            ...
            // (A->B)->(C->D) - позволят найти сначала среднюю
            if (s.charAt(i) == '(')
        ...
        for (int i = s.length() - 1; i > 0; --i) {
            ...
            if (i <= 0)
                break;
            // если == 0 то в пору бросать ексепшн.
            // если == -1, то значит строка начинается со скобки
            // далеко не факт, про -1. еще некореектность может быть
    ...
    private static String reduceBrackets(String s) {
        ...
        if (depth == 0 && i == s.length()) {
            // то есть скобки все же обрамляют строку, но мы не знаем, сколько их.
    ...
    private static int skipBrackets(String s) {
        ...
        if (i >= s.length()) return 0;    // типа если буква
    ...
    private static int skipBracketsLeft(String s) {
        ...
        try {
            ...
            System.out.println("PARSER::skipBracketsLeft->OutOfBound:");
